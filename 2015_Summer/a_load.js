...
(function(){
	...
	$contentChanged = #ajaxCenterContainerint, /** El contentedor que queremos cambiar **/
	...
	$.address.change(function(event){ 
		...
		if (event.value){
			...
			$content.empty().load(base + event.value + ' #ajaxCenterContainerint', function(){
				// $('.ContainerRight').prepend($('.SongComments'));
				$loader.fadeOut(300);
				$content.fadeIn('fast');
				$footer.show(0);
			});
